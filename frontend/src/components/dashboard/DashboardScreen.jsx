...
// ── CAMBIO: se agrega onPerfilActualizado a los props ──
export default function DashboardScreen({ userData, onLogout, onPerfilActualizado }) {
  ...
  useEffect(() => {
    if (!isAdmin) {
      if (adminView !== 'dashboard') setAdminView('dashboard');
      return;
    }
  ...
  const [resumen, setResumen] = useState({
    citasHoy: 0, pendientes: 0, atendidos: 0, canceladas: 0,
    consultoriosOcupados: 0, consultoriosTotales: 0, pacientes: 0, profesionales: 0,
  });
  const [weekly, setWeekly] = useState([]);
  const [stockItems, setStockItems] = useState([]);
  const [notifs, setNotifs] = useState([]);
  const [alertasSeguridad, setAlertasSeguridad] = useState({ criticas: 0, advertencias: 0, informativas: 0, ultimas: [] });
  ...
  const loadPanel = useCallback(async () => {
    try {
      ...
      setResumen({
        citasHoy: r.citas_totales ?? 0, pendientes: r.citas_pendientes ?? 0,
        atendidos: r.citas_atendidas ?? 0, canceladas: r.citas_canceladas ?? 0,
        consultoriosOcupados: r.consultorios_ocupados ?? 0, consultoriosTotales: r.consultorios_totales ?? 0,
        pacientes: r.pacientes_hoy ?? 0, profesionales: r.profesionales_activos ?? 0,
      });
      ...
      setStockItems(productos.map((p, idx) => ({ id: idx + 1, producto: p.nombre, stock_actual: p.stock_actual, stock_minimo: p.stock_minimo, nivel: p.nivel })));
      const mappedNotifs = (data.notificaciones ?? []).map((n, idx) => ({ id: idx + 1, tipo: n.tipo, mensaje: n.mensaje, accion: n.accion }));
      ...
      try {
        ...
        if (alertasRes.ok) {
          ...
          setAlertasSeguridad({ criticas: alertasData.total_criticas || 0, advertencias: alertasData.total_advertencias || 0, informativas: alertasData.total_informativas || 0, ultimas: alertasData.ultimas_alertas || [] });
        }
      } catch (err) { console.error('Error cargando resumen de alertas de seguridad:', err); }
  ...
  const topbarTitle = useMemo(() => {
    if (!isAdmin) return '';
    switch (adminView) {
      case 'consultorios':       return 'GESTIÓN DE CONSULTORIOS';
        ...
        <div className="dm2-colLeft">
          ...
          <CardSection title="Accesos rápidos" rightAction={<span style={{ fontSize: '12px' }}>Ahora disponible en barra superior</span>}>
            <div className="dm2-quickbar">
              {isAdmin ? (<>
                <button className="dm2-quickbtn" type="button" onClick={() => setAdminView('consultorios')}><i className="fa-solid fa-door-open" /><span>CONSULTORIOS</span></button>
                <button className="dm2-quickbtn" type="button" onClick={() => setAdminView('alertas-seguridad')}><i className="fa-solid fa-shield-alt" /><span>ALERTAS DE SEGURIDAD</span></button>
              </>) : null}
            ...
            <div className="dm2-notifs">
              {notifs.length === 0 ? <div className="dm2-empty">Sin notificaciones</div> : notifs.map((n) => {
                ...
                return (<div key={n.id} className="dm2-notifRow" role={n.accion ? 'button' : undefined} tabIndex={n.accion ? 0 : undefined} onClick={() => n.accion && (window.location.href = n.accion)} onKeyDown={(e) => n.accion && e.key === 'Enter' && (window.location.href = n.accion)} style={n.accion ? { cursor: 'pointer' } : undefined}>
                  <Dot variant={lvl} />
                  <div className="dm2-notifText"><span className="dm2-notifKind">{String(n.tipo || 'INFO').toUpperCase()}:</span> {n.mensaje}</div>
                </div>);
              })}
            </div>
          </CardSection>
  ...
  const renderMainContent = () => {
    if (!isAdmin) return <DentistDashboard userData={userData} onLogout={onLogout} />;
    if (adminView === 'consultorios')       return <ConsultoriosAdminScreen />;
    if (adminView === 'gestionar-cuentas')  return <GestionarCuentasScreen />;
    if (adminView === 'crear-cuenta')       return <CrearCuentaPlaceholder />;
    if (adminView === 'horarios')           return <HorariosAtencionScreen userData={userData} />;
    if (adminView === 'parametros')         return <ParametrosSistemaScreen userData={userData} />;
    if (adminView === 'monitoreo')          return <ErrorBoundary title="Error en Monitoreo" message="La pantalla de monitoreo encontró un error inesperado."><MonitoringScreen /></ErrorBoundary>;
    if (adminView === 'auditoria')          return <AuditScreen />;
    if (adminView === 'restauracion')       return <RestauracionScreen userData={userData} />;
    if (adminView === 'catalogo-insumos')   return <CatalogoInsumosScreen />;
    ...
    if (adminView !== 'dashboard') return (
      <div className="dm2-page"><div className="dm2-card">
        <div className="dm2-card-head"><div className="dm2-card-title">Vista: {adminView.toUpperCase()}</div><button className="dm2-linkBtn" type="button" onClick={() => setAdminView('dashboard')}>← Volver al panel de control</button></div>
        <div className="dm2-card-body"><div className="dm2-empty">Cargando configuración de {adminView}...</div></div>
      </div></div>
    );
                ...
                <div className="dm2-topbar-right">
                  <div className="dm2-alertas-dropdown-wrap">
                    <button className="dm2-alertas-secondary" onClick={() => setAlertasVisorAbierto((prev) => !prev)}>
                      <i className="fa-solid fa-bell" /> Alertas{' '}
                      {totalAlertasSeguridad + inventarioAlertCount > 0 && <span className="dm2-alertas-badge">{totalAlertasSeguridad + inventarioAlertCount}</span>}{' '}
                      <i className="fa-solid fa-chevron-down dm2-alertas-chevron" />
                    </button>
                    {alertasVisorAbierto && (
                      <div className="dm2-alertas-dropdown">
                        <button className="dm2-alertas-dropdown-item" onClick={() => { setAdminView('alertas-seguridad'); setAlertasVisorAbierto(false); }}>
                          <i className="fa-solid fa-shield-alt" /><span>Seguridad</span>
                          {totalAlertasSeguridad > 0 && <span className="dm2-alertas-badge dm2-alertas-badge--crit">{totalAlertasSeguridad}</span>}
                        </button>
                        <button className="dm2-alertas-dropdown-item" onClick={() => { setAdminView('alertas-inventario'); setAlertasVisorAbierto(false); }}>
                          <i className="fa-solid fa-boxes-stacked" /><span>Inventario</span>
                          {inventarioAlertCount > 0 && <span className="dm2-alertas-badge dm2-alertas-badge--warn">{inventarioAlertCount}</span>}
                        </button>
                      </div>
                    )}
                  </div>
