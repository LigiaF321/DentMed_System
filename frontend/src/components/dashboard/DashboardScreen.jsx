...
export default function DashboardScreen({ userData, onLogout, onUserDataUpdate }) {
  ...
  const loadPanel = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.error('Error cargando resumen de alertas de seguridad:', err);
      }
    } finally {
      setLoading(false);
    }
  ...
  const inventarioAlertCount = useMemo(
    () => stockItems.filter(item => item.nivel === 'crit' || item.nivel === 'warn').length,
    [stockItems]
  );
  ...
  const topbarTitle = useMemo(() => {
    ...
    switch (adminView) {
      case 'consultorios':
        return 'GESTIÓN DE CONSULTORIOS';
        ...
        <div className="dm2-colLeft">
          ...
          {isAdmin ? (
            <CardSection
              ...
              rightAction={
                <button className="dm2-linkBtn" onClick={() => setAdminView('alertas-seguridad')}>
                  Ver todas
                </button>
              }
            >
                  ...
                  {alertasSeguridad.ultimas.length > 0 ? (
                    alertasSeguridad.ultimas.slice(0, 3).map((alerta) => (
                      <div key={alerta.id} className="dm2-alertsWidget-item">
                        <span
                          className={`dm2-alertsWidget-dot dm2-alertsWidget-dot--${alerta.prioridad}`}
                        />
                        <span className="dm2-alertsWidget-text">
                          {alerta.descripcion}
                        </span>
                      </div>
                    ))
          ...
          <CardSection
            ...
            rightAction={<span style={{ fontSize: '12px' }}>Ahora disponible en barra superior</span>}
          >
            <div className="dm2-quickbar">
              {isAdmin ? (
                <>
                  <button className="dm2-quickbtn" type="button" onClick={() => setAdminView('consultorios')}>
                    <i className="fa-solid fa-door-open" />
                    <span>CONSULTORIOS</span>
                  </button>
                  <button className="dm2-quickbtn" type="button" onClick={() => setAdminView('alertas-seguridad')}>
          ...
          <CardSection
            ...
            rightAction={
              <button
                ...
                onClick={() => (isAdmin ? setAdminView('alertas-seguridad') : setAdminView('dashboard'))}
              >
                Ver centro →
              </button>
            }
  ...
  const renderMainContent = () => {
    ...
    if (adminView === 'monitoreo') {
      return (
        <ErrorBoundary
          title="Error en Monitoreo"
          message="La pantalla de monitoreo encontró un error inesperado."
        >
          <MonitoringScreen />
        </ErrorBoundary>
      );
                      ...
                      <div className="dm2-alertas-dropdown">
                        ...
                        >
                          ...
                          {totalAlertasSeguridad > 0 && (
                            <span className="dm2-alertas-badge dm2-alertas-badge--crit">
                              {totalAlertasSeguridad}
                            </span>
                          )}
                        </button>
                        ...
                        >
                          ...
                          {inventarioAlertCount > 0 && (
                            <span className="dm2-alertas-badge dm2-alertas-badge--warn">
                              {inventarioAlertCount}
                            </span>
                          )}
                        </button>
                      </div>
